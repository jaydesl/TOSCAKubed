        ...
        for node in sorted(nodes, key=lambda x: x.type, reverse=True):
            ...
            if DOCKER_CONTAINER in node.type and interface:                
                if '_' in node.name:
                    ...
                    raise ValueError("ERROR: Use of underscores in {} workload name prohibited".format(node.name))                      
                ...
                if '_' in name:
                    ...
                    raise ValueError("ERROR: Use of underscores in {} volume name prohibited".format(name))
